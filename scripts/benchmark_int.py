...
######################## needs to be checked from here##############################################################
        ...
        stats.update(task["flowId"], eval_result)
        print_task_progress(task["flowId"], task_result.num_steps, eval_result, stats)
        ...
        stats.update(task["flowId"], "failed")  # Mark as failed instead of crashing
        return
...
async def benchmark(result: AgentHistoryList,single_task: dict,max_concurrent_tasks: int, model_provider: str) -> None:
    try:
        # Setup
        ...
        async def process_with_semaphore(
            task: TaskData, client: AzureChatOpenAI | ChatAnthropic,history: AgentHistoryList
        ) -> None:
        ...
        await process_with_semaphore(single_task, model,result)
